...
const getSub = async (request, url, uuid) => {
    ...
    const processTemplate = (index) => {
        if (cachedTemplates[index]) {
            ...
            for (const ip of ipListAll) {
                finalLinks += tmpl.replaceAll("{{IP}}", ip) + "\n";
            }
        }
    };
    const addNodes = (base) => {
    ...
    if (hasVL) addNodes(0);
    if (hasTR) addNodes(4);
    if (finalLinks.length > 0) finalLinks = finalLinks.slice(0, -1);
    ...
    let target = '';
    if (url.searchParams.has(strList[5]) || UA.includes(strList[5]) || UA.includes(strList[15]) || UA.includes(strList[16])) {
        target = strList[5];
    } else if (url.searchParams.has(strList[11]) || url.searchParams.has(strList[6]) || UA.includes(strList[12]) || UA.includes(strList[6])) {
        target = strList[6];
    } else if (url.searchParams.has(strList[13]) || UA.includes(strList[13])) {
        target = strList[7];
    } else if (url.searchParams.has(strList[8]) || UA.includes(strList[14])) {
        target = strList[8];
    } else if (url.searchParams.has(strList[9]) || UA.includes(strList[9])) {
        target = strList[9];
    } else if (url.searchParams.has(strList[10]) || UA.includes(strList[10])) target = strList[10];
    if (target) {
        const baseUrl = `${url.protocol}//${url.host}${url.pathname}?uuid=${globalThis.subUuid}&format=raw&path=${encodeURIComponent(proxyPath)}&vl=${hasVL ? 1 : 0}&tj=${hasTR ? 1 : 0}&ws=${hasWS ? 1 : 0}&xhttp=${hasXhttp ? 1 : 0}`;
        ...
        try {
            const suffix = strList[2] + strList[3] + strList[2] + strList[4];
            const response = await fetch(convertUrl, {
                headers: {'User-Agent': strList[19] + ' for ' + target + ' ' + suffix}
            });
            if (response.ok) {
